...
if not db_profile:
        ...
        with st.form("quick_setup"):
            ...
            if st.form_submit_button("💾 Uložiť"):
                data = {
                    ...
                    "allergies": "", "dislikes": "", "coach_style": "Stručný", "archetype": "Pragmatik ⚡", "health_issues": "", 
                    "ai_strategy": "Základná stratégia: Dodržuj kalórie a hýb sa."
                }
    ...
    if st.session_state.onboarding_choice == "chat":
        ...
        if "onboarding_history" not in st.session_state:
            st.session_state.onboarding_history = [{"role": "model", "parts": [f"Čau {current_user}! Som Max. 🍎 Napíš mi: **Vek, výšku, váhu** a **Prečo chceš zmeniť postavu?**"]}]
        ...
        if submit_chat and user_input:
            ...
            with st.spinner("Max analyzuje..."):
                ...
                system_prompt = f"""
                Si Max, nutričný kouč. Audit klienta {current_user}. Zisti: 1. Fyzické parametre, 2. Životný štýl, 3. Chute.
                ...
                try:
                    ...
                    if "Ďakujem, mám všetko" in ai_reply:
                        with st.status("Generujem identitu...", expanded=True):
                            extract_prompt = f"""
                            ...
                            JSON: {{
                                ...
                                "archetype": "Názov (2 slová)", "health_issues": "", "ai_strategy": "5 viet."
                            }}
                            """
...
try:
    if db_profile and len(db_profile) > 14 and db_profile[14]:
        ...
        if (datetime.now() - last_up).days >= 7:
            with st.expander("⚠️ **Týždenná kontrola!**"):
                new_w = st.number_input("Nová váha (kg):", value=db_profile[3])
